(() => {
  chrome.runtime.onMessage.addListener(function (
    ...
    if (
      ...
      request.name === "DROPSHIPPING_TOOL"
    ) {
      const csvObjectArray = request?.csvObjectArray?.data;
      if (!(csvObjectArray && csvObjectArray.length)) return;
      injector(csvObjectArray, getOrderQueue());
    }
  });
  ...
  function injector(csvObjectArray, orderElementObject) {
    function injectTrackingDetail(orderIndex, tracking) {
      ...
      if (inputTracking && inputTracking instanceof HTMLElement) {
        ...
        inputTracking.dispatchEvent(new CustomEvent("change"));
      } else return;
    }

    function injectShippingMethodDetail(orderIndex, deliveryCode) {
    ...
    function orderDetailInjector(element, sku, qty) {
      function injectShippingWarehouseDetail(idTag) {
        const SELECT_WAREHOUSE_ID = idTag + ".shippingLineWarehouse";
        const OPTION_WAREHOUSE_ID =
          idTag + ".shippingLineWarehouse.Sterling-VA";
      ...
      function injectShipQuantityDetail(idTag, sku, qty) {
        ...
        if (elementSkuValue === sku) {
          ...
          elementQty.dispatchEvent(new CustomEvent("change")); //* for onchange event
        }
      }
      ...
      if (!(trArray && trArray.length)) return;
    ...
    if (csvObjectArray && orderElementObject) {
      csvObjectArray.forEach((csvObject) => {
        ...
        if (orderId.trim() === csvOrderId.trim()) {
          injectTrackingDetail(orderIndex, tracking);
          injectShippingMethodDetail(orderIndex, delivery_code);
          orderDetailInjector(element, sku, qty);
          element.click();
        }
      });
    }
